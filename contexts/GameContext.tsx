...
import {
  initGame, moveBoard, spawnTile, checkGameOver, checkWin, getLevelFromScore, destroyTile, clearAllBlockers,
} from '@/services/gameEngine';
import {
  getProfile, saveProfile, initOrLoadProfile, syncProfileFromSupabase, usePowerUp as consumePowerUp,
  saveBoardState, getSavedBoard, clearSavedBoard,
} from '@/services/storage';
import { startGameSession, settleGameSession } from '@/services/gameSession';
import { Tile, PlayerProfile, PowerUpType } from '@/types/game';
...
export function GameProvider({ children }: { children: ReactNode }) {
  ...
  const [earningMultiplier, setEarningMultiplier] = useState(1); const [multiplierSecondsLeft, setMultiplierSecondsLeft] = useState(0); const [multiplierAdsWatched, setMultiplierAdsWatched] = useState(0);
  ...
  useEffect(() => {
    if (multiplierSecondsLeft <= 0) { if (earningMultiplier !== 1) setEarningMultiplier(1); return; }
    const timer = setInterval(() => setMultiplierSecondsLeft(v => Math.max(0, v - 1)), 1000);
    return () => clearInterval(timer);
  }, [multiplierSecondsLeft, earningMultiplier]);

  useEffect(() => { if (multiplierSecondsLeft === 0) setMultiplierAdsWatched(0); }, [multiplierSecondsLeft]);
  ...
  const beginServerSession = useCallback(async () => {
    try {
      const session = await startGameSession();
      gameSessionRef.current = session.clientSessionId;
    } catch (error) {
      gameSessionRef.current = null;
      if (process.env.NODE_ENV !== 'production') console.warn('Unable to start server game session:', error);
    }
  }, []);
  ...
  const loadProfile = async () => {
    ...
    setProfileState(p); setBestScore(p.bestScore); await beginServerSession();
  };
  ...
  const recordMultiplierAd = useCallback(() => {
    setMultiplierAdsWatched(current => {
      ...
      if (next === 1) { setEarningMultiplier(2); setMultiplierSecondsLeft(MULTIPLIER_DURATION_SECONDS); }
      else if (next >= MULTIPLIER_TARGET_ADS) { setEarningMultiplier(3); setMultiplierSecondsLeft(MULTIPLIER_DURATION_SECONDS); }
      return next;
    });
  }, []);

  const settleCurrentSession = useCallback(async (finalBoard: (Tile | null)[][], finalScore: number, finalMoves: number, finalLevel: number) => {
    const clientSessionId = gameSessionRef.current;
    if (!clientSessionId || finalMoves <= 0) return;
    const maxTile = Math.max(2, ...finalBoard.flat().map(tile => tile?.type === 'normal' ? tile.value : 0));
    const settlement = await settleGameSession(clientSessionId, finalScore, finalMoves, finalLevel, maxTile, finalBoard);
    if (settlement) {
      setSessionTokens(settlement.reward);
      if (settlement.levelReward > 0) setLevelUpReward(settlement.levelReward);
  ...
  const move = useCallback((dir: 'up' | 'down' | 'left' | 'right') => {
    ...
    setBoard(prev => {
      ...
      setSessionTokens(st => Math.round((st + Math.max(0, gained * 0.12)) * 100) / 100);
      movesRef.current += 1; const newMoves = movesRef.current; setMoves(newMoves);
      const { board: withSpawn } = spawnTile(moved, newScore, newMoves);
      ...
      if (checkGameOver(withSpawn)) {
        ...
        if (preGameOverState) saveBoardState(preGameOverState); else saveBoardState({ board: withSpawn, score: newScore, moves: newMoves });
        void settleCurrentSession(withSpawn, newScore, newMoves, newLevel);
        setCanContinue(true);
      }
      boardRef.current = withSpawn; return withSpawn;
    });
  }, [isGameOver, isWon, isSelectingDestroy, bestScore, settleCurrentSession]);
  ...
  const continueFromSaved = useCallback(async () => {
    try {
      const saved = await getSavedBoard(); if (!saved) { newGame(); return; }
      const restoredBoard = saved.board as (Tile | null)[][]; const restoredScore = saved.score || 0; const restoredMoves = saved.moves || 0;
      await clearSavedBoard(); boardRef.current = restoredBoard; scoreRef.current = restoredScore; movesRef.current = restoredMoves;
      undoStack.current = []; setBoard(restoredBoard); setScore(restoredScore); setMoves(restoredMoves); setLevel(getLevelFromScore(restoredScore)); setSessionTokens(0); setCanContinue(false); setIsWon(false);
      setTimeout(() => setIsGameOver(false), 50); void beginServerSession();
    } catch { newGame(); }
  }, [newGame, beginServerSession]);

  const dismissLevelUp = useCallback(() => setLevelUpReward(null), []);
  const dismissNewTier = useCallback(() => setNewTierValue(null), []);
  const setProfile = useCallback((p: PlayerProfile) => { setProfileState(p); saveProfile(p); }, []);
