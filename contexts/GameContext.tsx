...
import {
  ...
  saveBoardState, getSavedBoard, clearSavedBoard, syncProfileFromSupabase,
...
export function GameProvider({ children }: { children: ReactNode }) {
  ...
  const loadSeenTiles = async () => {
    try {
      ...
      if (raw) seenTileValues.current = new Set(JSON.parse(raw) as number[]);
    } catch {}
  };

  const markTileSeen = async (value: number) => {
    seenTileValues.current.add(value);
    try { await AsyncStorage.setItem(SEEN_TILES_KEY, JSON.stringify(Array.from(seenTileValues.current))); } catch {}
  };
  ...
  const loadProfile = async () => {
    // Never create a local/random identity. Telegram initData is the only
    // authoritative identity for a production Mini App.
    await waitForTelegramWebApp();
    const tgUser = detectTelegramUser();
    if (!tgUser) {
      throw new Error('Telegram user identity is unavailable. Please reopen MintGrow from Telegram.');
    }
  ...
  const refreshProfile = useCallback(async () => {
    ...
    if (!current?.telegramId) return;
    const fresh = await syncProfileFromSupabase(current.telegramId);
    if (fresh) {
      setProfileState(fresh);
      setBestScore(fresh.bestScore);
    }
  }, [profile]);
  ...
  const move = useCallback((dir: 'up' | 'down' | 'left' | 'right') => {
    ...
    setBoard(prev => {
      ...
      if (checkGameOver(withSpawn)) {
        ...
        if (preGameOverState) saveBoardState(preGameOverState);
        else saveBoardState({ board: withSpawn, score: scoreRef.current, moves: movesRef.current });
        setCanContinue(true);
      }
  ...
  const newGame = useCallback(() => {
    ...
    setBoard(fresh); boardRef.current = fresh; setScore(0); scoreRef.current = 0; setSessionTokens(0); setMoves(0); movesRef.current = 0;
  ...
  const continueFromSaved = useCallback(async () => {
    try {
      ...
      boardRef.current = restoredBoard; scoreRef.current = restoredScore; movesRef.current = restoredMoves; undoStack.current = [];
      setBoard(restoredBoard); setScore(restoredScore); setMoves(restoredMoves); setLevel(getLevelFromScore(restoredScore)); setSessionTokens(0); setCanContinue(false); setIsWon(false);
      setTimeout(() => setIsGameOver(false), 50);
    } catch { newGame(); }
  }, [newGame]);
  ...
  const activatePowerUp = useCallback(async (type: PowerUpType): Promise<boolean> => {
    ...
    if (type === 'undo') {
      ...
      setProfileState(updated); setBoard(prev.board); boardRef.current = prev.board; setScore(prev.score); scoreRef.current = prev.score; setMoves(prev.moves); movesRef.current = prev.moves; setIsGameOver(false); setLevel(getLevelFromScore(prev.score));
      return true;
    }
    ...
    if (type === 'clear_blockers') {
      ...
      if (!updated) return false;
      setProfileState(updated); setBoard(prev => { const cleared = clearAllBlockers(prev); boardRef.current = cleared; return cleared; });
      return true;
    }
    if (type === 'shuffle') {
      const updated = await consumePowerUp('shuffle');
      if (!updated) return false;
      setProfileState(updated); setBoard(prev => {
        const tiles: Tile[] = [];
        for (let r = 0; r < 4; r++) for (let c = 0; c < 4; c++) if (prev[r][c]) tiles.push(prev[r][c]!);
        for (let i = tiles.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [tiles[i], tiles[j]] = [tiles[j], tiles[i]]; }
        const nb: (Tile | null)[][] = Array(4).fill(null).map(() => Array(4).fill(null));
        let ti = 0;
        for (let r = 0; r < 4; r++) for (let c = 0; c < 4; c++) if (ti < tiles.length) nb[r][c] = { ...tiles[ti++], row: r, col: c, isNew: true, isMerged: false };
        boardRef.current = nb; return nb;
      });
      return true;
    }
    return false;
  ...
  const selectTileToDestroy = useCallback(async (row: number, col: number) => {
    ...
    setBoard(prev => { const nb = destroyTile(prev, row, col); boardRef.current = nb; return nb; });
  ...
  return <GameContext.Provider value={{ board, score, bestScore, level, tokens, sessionTokens, moves, isGameOver, isWon, profile, levelUpReward, newTierValue, isSelectingDestroy, canContinue, move, newGame, continueGame, continueFromSaved, dismissLevelUp, dismissNewTier, setProfile, refreshProfile, activatePowerUp, selectTileToDestroy, cancelDestroy }}>{children}</GameContext.Provider>;
}
