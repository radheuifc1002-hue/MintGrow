...
import { getWithdrawals, updateWithdrawal, getProfile } from '@/services/storage';
...
export default function AdminScreen() {
  ...
  const handleApprove = async () => {
    ...
    if (!txHashInput.trim()) {
      Alert.alert('Required', 'Please enter the transaction hash');
      return;
    }
    await updateWithdrawal(selectedRequest.id, {
      status: 'approved',
      txHash: txHashInput.trim(),
      processedAt: new Date().toISOString(),
    });

    // Update profile withdrawn tokens
    const p = await getProfile();
    if (p) {
      p.withdrawnTokens = Math.round((p.withdrawnTokens + selectedRequest.amount) * 100) / 100;
      p.pendingTokens = Math.max(0, Math.round((p.pendingTokens - selectedRequest.amount) * 100) / 100);
      const { saveProfile } = await import('@/services/storage');
      await saveProfile(p);
    }
    ...
    setTxHashInput('');
    loadData();
    Alert.alert('✅ Approved', 'Withdrawal has been approved and TX hash recorded');
  };
  ...
  const handleReject = async () => {
    ...
    Alert.alert(
      'Reject Withdrawal',
      'This will refund tokens to the user. Continue?',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            const p = await getProfile();
            if (p) {
              p.totalTokens = Math.round((p.totalTokens + selectedRequest.amount) * 100) / 100;
              p.pendingTokens = Math.max(0, Math.round((p.pendingTokens - selectedRequest.amount) * 100) / 100);
              const { saveProfile } = await import('@/services/storage');
              await saveProfile(p);
            }
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      {pendingCount > 0 && (
        <View style={styles.alertBar}>
          ...
          <Text style={styles.alertText}>{pendingCount} withdrawal{pendingCount > 1 ? 's' : ''} pending approval</Text>
        </View>
      )}
      ...
      {tab === 'stats' ? (
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          <View style={styles.statsGrid}>
            <StatBox label="Pending MG" value={totalPending.toFixed(2)} color={Colors.warning} />
            <StatBox label="Approved MG" value={totalApproved.toFixed(2)} color={Colors.success} />
            ...
            <StatBox label="Player Score" value={(profile?.bestScore ?? 0).toLocaleString()} color={Colors.accent} />
            <StatBox label="Ads Watched" value={profile?.adsWatched ?? 0} color={Colors.primary} />
          </View>

          <View style={styles.infoCard}>
            <Text style={styles.infoTitle}>💡 Admin Actions</Text>
            <Text style={styles.infoBody}>
              1. Review pending withdrawal requests{'\n'}
              2. Connect your wallet to send MG tokens{'\n'}
              3. Enter the transaction hash to confirm{'\n'}
              4. Approve or reject each request manually{'\n'}
              5. Rejected requests automatically refund tokens
            </Text>
          </View>
        ...
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          {filteredWithdrawals.length === 0 ? (
            ...
            filteredWithdrawals.map(w => (
              <Pressable key={w.id} style={styles.requestCard} onPress={() => setSelectedRequest(w)}>
                ...
                <Text style={styles.requestAmount}>{w.amount.toFixed(2)} MG</Text>
                <Text style={styles.requestWallet} numberOfLines={1}>
                  → {w.walletAddress}
                </Text>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <View style={styles.modalInfo}>
              <InfoRow label="User" value={`@${selectedRequest?.username}`} />
              <InfoRow label="Amount" value={`${selectedRequest?.amount.toFixed(2)} MG`} />
              <InfoRow label="Wallet" value={selectedRequest?.walletAddress ?? ''} mono />
              <InfoRow label="Date" value={new Date(selectedRequest?.createdAt ?? '').toLocaleString()} />
            </View>

            {selectedRequest?.status === 'pending' ? (
              <>
                <Text style={styles.txLabel}>Transaction Hash (after sending):</Text>
                <TextInput
              ...
              <View style={[styles.processedBanner, { borderColor: statusColor(selectedRequest?.status ?? '') }]}>
                <Text style={[styles.processedText, { color: statusColor(selectedRequest?.status ?? '') }]}>
                  {selectedRequest?.status?.toUpperCase()} on {new Date(selectedRequest?.processedAt ?? '').toLocaleString()}
                </Text>
                {selectedRequest?.txHash && (
                  <Text style={styles.txHashFull}>TX: {selectedRequest.txHash}</Text>
                )}
              </View>
            )}

            <Pressable style={styles.closeBtn} onPress={() => { setSelectedRequest(null); setTxHashInput(''); }}>
...
function StatBox({ label, value, color }: { label: string; value: any; color: string }) {
  return (
    <View style={[sbStyles.box, { borderColor: color }]}>
...
function InfoRow({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <View style={irStyles.row}>
      ...
      <Text style={[irStyles.value, mono && irStyles.mono]} numberOfLines={2}>{value}</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  activeChipText: { color: Colors.bg, fontWeight: '700' },
  ...
  requestAmount: { fontSize: 22, fontWeight: '800', color: Colors.primary },
