...
async function hmac(key: ArrayBuffer | CryptoKey, data: string): Promise<ArrayBuffer> {
  const cryptoKey = key instanceof CryptoKey ? key : await crypto.subtle.importKey('raw', key, { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']);
  return crypto.subtle.sign('HMAC', cryptoKey, new TextEncoder().encode(data));
}
async function verifyTelegramInitData(initData: string, botToken: string) {
  const params = new URLSearchParams(initData);
  const suppliedHash = params.get('hash');
  if (!initData || !botToken || !suppliedHash) throw new Error('telegram_identity_unavailable');
  params.delete('hash');
  const check = [...params.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([k, v]) => `${k}=${v}`).join('\n');
  const secret = await hmac(new TextEncoder().encode('WebAppData').buffer, botToken);
  if (hex(await hmac(secret, check)) !== suppliedHash) throw new Error('telegram_signature_invalid');
  ...
  const user = JSON.parse(raw) as { id?: number; username?: string; first_name?: string; photo_url?: string };
  if (!user.id) throw new Error('telegram_user_missing');
  return user;
}

const ACTIONS = new Set([
  'get_player','ensure_player','ensure_referral_code','complete_registration','update_profile_metadata',
  'credit_player_tokens','record_ad_event','record_game_session','apply_referral_code','submit_withdrawal_request',
  'get_referrals','get_withdrawals','get_leaderboard','get_player_rank','claim_daily_bonus','spend_tokens_for_powerup',
]);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'get_referrals') {
      const { data, error } = await db.from('referrals').select('referrer_telegram_id,referee_telegram_id,level,tokens_earned,created_at,players!referrals_referee_telegram_id_fkey(username,total_tokens)').eq('referrer_telegram_id', telegramId).order('created_at', { ascending: false }).limit(100);
      if (error) return json({ error: error.message }, 400);
    ...
    if (action === 'get_player_rank') {
      const { data: player, error: pe } = await db.from('players').select('total_tokens').eq('telegram_id', telegramId).maybeSingle();
