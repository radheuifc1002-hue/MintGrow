...
Deno.serve(async (req) => {
  ...
  try {
    ...
    stage = 'load_server_config'; const botToken = Deno.env.get('TELEGRAM_BOT_TOKEN'); const serviceRole = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY'); const supabaseUrl = Deno.env.get('SUPABASE_URL'); if (!botToken || !serviceRole || !supabaseUrl) return json({ error: 'server_not_configured' }, 500);
    stage = 'verify_telegram'; const telegramUser = await verifyTelegramInitData(String(body?.initData ?? ''), botToken); const telegramId = String(telegramUser.id); const db = createClient(supabaseUrl, serviceRole, { auth: { persistSession: false } }); const p = body?.params ?? {};
    if (String(p.telegramId ?? p.p_telegram_id ?? telegramId) !== telegramId) return json({ error: 'telegram_identity_mismatch' }, 403);
    if (action === 'get_player') { stage = 'get_player'; const { data, error } = await db.from('players').select('*').eq('telegram_id', telegramId).maybeSingle(); if (error) return json({ error: error.message }, 400); return json({ data, telegramId }); }
    if (action === 'get_referrals') { stage = 'get_referrals'; const { data, error } = await db.from('referrals').select('referrer_telegram_id,referee_telegram_id,level,tokens_earned,created_at,players!referrals_referee_telegram_id_fkey(username,total_tokens)').eq('referrer_telegram_id', telegramId).order('created_at', { ascending: false }).limit(100); if (error) return json({ error: error.message }, 400); return json({ data: data ?? [], telegramId }); }
    if (action === 'get_withdrawals') { stage = 'get_withdrawals'; const { data, error } = await db.from('withdrawals').select('*').eq('telegram_id', telegramId).order('created_at', { ascending: false }).limit(50); if (error) return json({ error: error.message }, 400); return json({ data: data ?? [], telegramId }); }
    if (action === 'get_leaderboard') { stage = 'get_leaderboard'; const limit = Math.min(Math.max(Number(p.limit ?? 50), 1), 100); const { data, error } = await db.from('players').select('telegram_id,username,total_tokens,level,best_score').order('total_tokens', { ascending: false }).order('best_score', { ascending: false }).limit(limit); if (error) return json({ error: error.message }, 400); return json({ data: data ?? [], telegramId }); }
    if (action === 'get_player_rank') { stage = 'get_player_rank'; const { data: player, error: pe } = await db.from('players').select('total_tokens').eq('telegram_id', telegramId).maybeSingle(); if (pe) return json({ error: pe.message }, 400); if (!player) return json({ data: null, telegramId }); const { count, error } = await db.from('players').select('telegram_id', { count: 'exact', head: true }).gt('total_tokens', player.total_tokens ?? 0); if (error) return json({ error: error.message }, 400); return json({ data: (count ?? 0) + 1, telegramId }); }
    ...
    stage = `rpc:${rpc}`; const { data, error } = await db.rpc(rpc,args); if (error) return json({ error:error.message },400); return json({ data, telegramId });
