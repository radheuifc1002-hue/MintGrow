...
async function verifyTelegramInitData(initData: string, botToken: string) {
  ...
  const raw = params.get('user'); if (!raw) throw new Error('telegram_user_missing');
  const user = JSON.parse(raw) as { id?: number; username?: string; first_name?: string; photo_url?: string };
  if (!user.id) throw new Error('telegram_user_missing'); return user;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const botToken = Deno.env.get('TELEGRAM_BOT_TOKEN'); const serviceRole = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY'); const supabaseUrl = Deno.env.get('SUPABASE_URL');
    if (!botToken || !serviceRole || !supabaseUrl) return json({ error: 'server_not_configured' }, 500);
    const telegramUser = await verifyTelegramInitData(String(body?.initData ?? ''), botToken); const telegramId = String(telegramUser.id);
    const db = createClient(supabaseUrl, serviceRole, { auth: { persistSession: false } }); const p = body?.params ?? {};
