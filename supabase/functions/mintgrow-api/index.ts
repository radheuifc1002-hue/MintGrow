...
async function verifyTelegramInitData(initData: string, botToken: string) {
  ...
  const authDate = Number(params.get('auth_date') ?? 0); const age = Math.floor(Date.now() / 1000) - authDate;
  if (!Number.isFinite(authDate) || age < -60 || age > 3600) throw new Error('telegram_init_data_expired');
