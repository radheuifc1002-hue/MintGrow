...
serve(async (req) => {
  ...
  // Never process a Telegram update until Telegram's webhook secret is verified.
  if (!WEBHOOK_SECRET) {
    console.error('TELEGRAM_WEBHOOK_SECRET is not configured');
    return new Response('server_not_configured', { status: 503, headers: corsHeaders });
  }
  ...
  try {
    ...
    if (message) {
      ...
      if (!Number.isSafeInteger(fromId)) return new Response('ok', { headers: corsHeaders });
      ...
      if (text.startsWith('/start')) {
        ...
        if (!player) {
          // Registration/referral economics are now delegated to the same
          // transactional database flow used by the Mini App.
          const { data: ensured, error: ensureError } = await supabase.rpc('ensure_player', {
        ...
        if (referralCode && !player.referred_by) {
          const { error: referralError } = await supabase.rpc('apply_referral_code', {
            p_referee_telegram_id: telegramId,
            p_code: referralCode,
          });
          if (referralError) console.warn('Referral application failed:', referralError.message);
          player = await getPlayer(telegramId) || player;
        }
        ...
        if (!player) await sendMessage(chatId, '❌ Account not found. Send /start to register.');
        else {
    ...
    if (update.callback_query) {
      ...
      if (!cqChatId) return new Response(JSON.stringify({ ok: true }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });

      if (cq.data === 'balance') {
        const player = await getPlayer(cqUserId);
        if (player) await sendMessage(cqChatId, `💰 Balance: <b>${parseFloat(player.total_tokens ?? '0').toLocaleString()} MG</b> | Level: <b>${player.level}</b>`);
      }
      if (cq.data === 'referrals') {
        const player = await getPlayer(cqUserId);
        if (player) await sendMessage(cqChatId, `👥 Direct refs: <b>${player.direct_referral_count ?? 0}</b>\n💎 Ref earnings: <b>${parseFloat(player.referral_tokens_earned ?? '0').toLocaleString()} MG</b>\n\nCode: <code>${player.referral_code}</code>`);
      }
      await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/answerCallbackQuery`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ callback_query_id: cq.id }),
      });
    }
    ...
    return new Response(JSON.stringify({ error: 'request_failed' }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  }
});
