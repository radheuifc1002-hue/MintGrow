...
begin
  if nullif(trim(p_client_session_id), '') is null or length(p_client_session_id) > 128 then
    raise exception 'Invalid game session id';
  end if;
  if not exists (select 1 from players where telegram_id = p_telegram_id) then
...
begin
  ...
  if session_row.status <> 'active' or session_row.settled_at is not null then
  ...
  foreach i in array generate_series(1,8) loop
    if p_score >= score_thresholds[i] and player_row.level < i then
      level_reward := level_reward + level_rewards[i];
    end if;
  end loop;
  derived_level := 1;
  ...
  old_level := player_row.level;

  -- Deterministic server-side gameplay reward. The client reward calculation is
  -- deliberately ignored. Daily cap remains the final economic boundary.
