...
create or replace function public.start_game_session(p_telegram_id text,p_client_session_id text)
...
begin
  ...
  on conflict(client_session_id) do nothing returning * into row_session;
...
declare
  session_row public.game_sessions; player_row public.players; counter public.reward_daily_counters;
  before_balance numeric; gameplay_reward numeric; level_reward numeric:=0; accepted numeric; remaining numeric; derived_level integer:=1; i integer;
...
begin
  ...
  select * into session_row from game_sessions where client_session_id=trim(p_client_session_id) and telegram_id=p_telegram_id for update;
  ...
  -- The submitted level and reward amount are never trusted.
  for i in 1..8 loop
  ...
  insert into reward_daily_counters(telegram_id,reward_date) values(p_telegram_id,(now() at time zone 'UTC')::date) on conflict(telegram_id,reward_date) do nothing;
  select * into counter from reward_daily_counters where telegram_id=p_telegram_id and reward_date=(now() at time zone 'UTC')::date for update;
  remaining:=greatest(0,5000-counter.gameplay_tokens); accepted:=least(accepted,remaining);
  ...
  update reward_daily_counters set gameplay_tokens=gameplay_tokens+accepted where telegram_id=p_telegram_id and reward_date=(now() at time zone 'UTC')::date;
