...
export const getEligibleReferralPct = (level: number, directRefs: number): number => {
  ...
  if (!rule || directRefs < rule.directRequired) return 0;
  return rule.pct;
};

// Power-up controls use Material Icons rather than Unicode/emoji glyphs.
export const POWER_UPS: PowerUp[] = [
  { type: 'undo', label: 'Undo Move', icon: 'undo', description: 'Reverse your last move', adRequired: true, tokenCost: 500 },
  { type: 'destroy', label: 'Destroy Tile', icon: 'delete-outline', description: 'Remove any one tile from board', adRequired: true, tokenCost: 1000 },
