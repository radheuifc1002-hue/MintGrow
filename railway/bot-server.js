...
function requireConfig() {
  ...
  if (!MINI_APP_URL) missing.push('MINI_APP_URL');
  return missing;
}
...
async function answerCallbackQuery(id) { return telegram('answerCallbackQuery', { callback_query_id: id }); }
...
async function registerNewPlayer(telegramId, username, referralCode) {
  ...
  const insertData = { telegram_id: telegramId, username, referral_code: newCode, total_tokens: 100, is_registered: false };
  if (referralCode) {
    const normalizedCode = referralCode.toUpperCase();
    const { data: referrer } = await supabase.from('players').select('telegram_id, direct_referral_count, total_tokens').eq('referral_code', normalizedCode).maybeSingle();
    if (referrer && referrer.telegram_id !== telegramId) {
      insertData.referred_by = normalizedCode;
      await supabase.from('players').update({ direct_referral_count: (referrer.direct_referral_count ?? 0) + 1, total_tokens: Number.parseFloat(referrer.total_tokens ?? '0') + 500 }).eq('telegram_id', referrer.telegram_id);
      await supabase.from('referrals').upsert({ referrer_telegram_id: referrer.telegram_id, referee_telegram_id: telegramId, level: 1, tokens_earned: 0 }, { onConflict: 'referrer_telegram_id,referee_telegram_id' });
    }
  }
...
async function handleMessage(message) {
  ...
  if (text === '/referral' || text === '/invite') {
    ...
    if (!player) { await sendMessage(chatId, '❌ Not found. Send /start to register.'); return; }
    const code = player.referral_code;
    await sendMessage(chatId, `👥 <b>Referrals</b>\n\nCode: <code>${code}</code>\nDirect: <b>${player.direct_referral_count ?? 0}</b>\nEarnings: <b>${Number.parseFloat(player.referral_tokens_earned ?? '0').toLocaleString()} MG</b>\n\nLink: ${referralLink(code)}`);
    return;
  }

  if (text === '/help') {
    await sendMessage(chatId, '🤖 <b>MintGrow Commands</b>\n\n/start — Register & launch\n/balance — Check MG balance\n/referral — Your referral code\n/help — This menu', { reply_markup: miniAppKeyboard('🎮 Play Now') });
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && url.pathname === '/') return json(res, { status: 'ok' }, { status: 'ok', service: 'MintGrow Telegram Bot', timestamp: new Date().toISOString(), missingEnv: requireConfig() });
  if (req.method === 'GET' && url.pathname === '/health') {
    const missingEnv = requireConfig();
    return json(res, missingEnv.length ? 500 : 200, { status: missingEnv.length ? 'misconfigured' : 'healthy', missingEnv });
  }
  if (req.method === 'GET' && url.pathname === '/setup-webhook') {
    ...
    const telegramResponse = await telegram('setWebhook', { url: webhookUrl, allowed_updates: ['message', 'edited_message', 'callback_query'], drop_pending_updates: true });
...
server.listen(PORT, () => console.log(`MintGrow Bot server running on port ${PORT}`));
